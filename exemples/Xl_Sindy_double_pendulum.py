...
ideal_solution_vector = create_solution_vector(sp.expand_trig(L.subs(substitutions)), full_catalog, friction_terms=friction_forces)
...
# External forces
external_force_func = optimized_force_generator(num_coordinates, max_force_span, simulation_end_time, time_period, time_shift, augmentations=15)
...
time_values, phase_values = run_rk45_integration(dynamics_system, initial_conditions, simulation_end_time, max_step=0.01)
theta_values = phase_values[:, ::2]
velocity_values = phase_values[:, 1::2]
acceleration_values = np.gradient(phase_values[:, 1::2], time_values, axis=0, edge_order=2)

# Regression
num_time_points = len(time_values)
subsample_rate = num_time_points // (scale_factor * catalog_length)
ideal_model_expression = create_solution_expression(ideal_solution_vector[:, 0], full_catalog, friction_count=len(friction_forces))

print("Ideal Model:", ideal_model_expression)

solution, experiment_matrix, subsample_time_values, _ = execute_regression(
    time_values, theta_values, time_sym, symbols_matrix, full_catalog,
    external_force_func, subsample_rate=subsample_rate, velocity_values=velocity_values,
    acceleration_values=acceleration_values, use_regression=use_regression, hard_threshold=3e-3)

if use_regression:
    error = np.linalg.norm(solution / np.max(solution) - ideal_solution_vector / np.max(ideal_solution_vector)) / np.linalg.norm(ideal_solution_vector / np.max(ideal_solution_vector))
...
if solution is not None:
    ...
    validated_time_values, validated_theta_values = run_rk45_integration(dynamics_system_validated, initial_conditions, simulation_end_time, max_step=0.05)
    axs[0, 0].plot(validated_time_values, validated_theta_values[:, 0], "--", label="Model - Reg. Classic")
    axs[1, 0].plot(validated_time_values, validated_theta_values[:, 2], "--", label="Model - Reg. Classic")
...
axs[2, 0].plot(np.repeat(subsample_time_values, num_coordinates) * 2, (experiment_matrix @ ideal_solution_vector - force_vector), label="Ideal Solution")

if use_regression:
    axs[2, 0].plot(np.repeat(subsample_time_values, num_coordinates) * 2, (experiment_matrix @ solution - force_vector), label="Solution")

axs[2, 1].set_title("Model Comparison")
bar_height_ideal = np.abs(solution) / np.max(np.abs(solution))
axs[2, 1].bar(np.arange(len(ideal_solution_vector)), bar_height_ideal[:, 0], width=1, label="True Model")

if use_regression :
    bar_height_found = np.abs(solution) / np.max(np.abs(solution))
    axs[2, 1].bar(np.arange(len(ideal_solution_vector)), bar_height_found[:, 0], width=0.5, label="Model Found")
