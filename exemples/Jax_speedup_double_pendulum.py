...
for i in range(sample):
    random_vec = np.random.random((symbols_matrix.shape[1]*2,)+ (parralel_batch,))*5 # generate a batch of ([q0,q0_d,...,qn,qn_d],...) vector
    random_forces = np.random.random((symbols_matrix.shape[1],)+ (parralel_batch,))*5 # generate a batch of forces ([f0,...,fn],...) vector
...
ts = np.array(ts)
print(f"Execution time dyn_function: {ts.mean()*1000:.6f} ms relative sequential: {ts.mean()*1000/parralel_batch:.6f} ms")
print(f"min time dyn_function: {ts.min()*1000:.6f} ms relative sequential: {ts.min()*1000/parralel_batch:.6f} ms")
