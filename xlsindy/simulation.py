...
def execute_regression(
    ...
    forces_vector = calculate_forces_vector(external_force_function, sampled_time_values)
    ...
    if use_regression:
        ...
        normalized_matrix, reduction_indices, variance_vector = normalize_experiment_matrix(
            experimental_matrix, 
            null_effect=apply_normalization
        )
        ...
        solution = unnormalize_experiment(coefficients, variance_vector, reduction_indices, experimental_matrix)
        ...
        covariance_matrix[nonzero_indices[:, np.newaxis], nonzero_indices] = covariance_reduced

        residuals = forces_vector - experimental_matrix @ solution
        sigma_squared = 1 / (experimental_matrix.shape[0] - experimental_matrix.shape[1]) * residuals.T @ residuals
