...
def mujoco_align_cmd_creator(exp,max_time,forces_scale,random_seeds):
    ...
    return     [
        ...
        "--no-real_mujoco_time",
        "--forces-scale-vector",*[str(num) for num in forces_scale],
        "--forces-period", "3",
        "--forces-period-shift", "0.5",
        "--regression",
        "--random-seed",*[str(seed) for seed in random_seeds]
    ]

def mujoco_generate_cmd_creator(exp,max_time,forces_scale,random_seeds,sample_number):
    ...
    return     [
        ...
        "--forces-period-shift", "0.5",
        "--random-seed",*[str(seed) for seed in random_seeds],
        "--sample-number",str(sample_number)
    ]
...
if __name__=="__main__":
    ...
    if args.number_coordinate == "None":
        raise ValueError("You need to specify the number of coordinate of your environment, don't hesitate to invoke --help")
    if len(args.forces_span) != 2:
        raise ValueError("Force span should have only one start and one end (lenght of 2), don't hesitate to invoke --help")
    if args.number_coordinate ==0:
        raise ValueError("Number of coordinate should be non null, don't hesitate to invoke --help")
    ...
    for i in range(args.number_experiment):
        ...
        forces_vec = rng.random(args.number_coordinate)*(i/args.number_experiment*(args.forces_span[1]-args.forces_span[0]) + args.forces_span[0])

        if args.mode =="align" :
            commands += [mujoco_align_cmd_creator(args.experiment_folder,args.max_time,forces_vec,[args.random_seed,i])]
