...
for i, couple in enumerate(couples):
    ...
    for pos, noise in zip(pos_list, noise_levels):
        ...
        if label not in legend_entries:
            legend_entries[label] = plt.Line2D([], [], color=color, marker='s',
...
ax.set_title('Box Plot of RMSE_validation for each (algoritm, optimization_function) couple by noise level')
ax.legend(legend_entries.values(), legend_entries.keys(), bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.show()
