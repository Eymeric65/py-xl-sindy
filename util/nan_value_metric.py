...
pairs = df[['optimization_function', 'input_experiment_folder']].drop_duplicates().sort_values(
...
fig, axes = plt.subplots(n_rows, total_cols, figsize=(2.5 * total_cols, 2.5 * n_rows), sharey=False)
...
for row_idx, (func, folder) in enumerate(pairs_list):
    for noise_idx, noise in enumerate(noise_values):
        for metric_idx, metric in enumerate(metric_cols):
            ...
            bp = ax.boxplot([data_nan, data_non, data_low],
                            positions=[1, 2, 3],
                            widths=0.6,
                            patch_artist=True,
                            showfliers=False)
            ...
            if row_idx == 0:
                ...
                ax.annotate(metric, xy=(0.5, 1.05), xycoords='axes fraction',
