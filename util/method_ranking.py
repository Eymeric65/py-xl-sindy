...
mask_main = ((df_noise0['optimization_function'] == 'hard_threshold_sparse_regression') &
...
for (alg, opt_func, noise), group in groups:
    ...
    plt.plot(group['x_coord'], group['RMSE_validation'], label=label,
             color=color, linestyle=linestyle, marker='o')

plt.xlabel('Experiment (ordered by RMSE_validation from base: hard_threshold_sparse_regression, xlsindy, noise=0)')
